...
import com.katariastoneworld.apis.dto.BillCancelPreviewDTO;
...
public class BillController {
    ...
    @Autowired
    private BillRevisionService billRevisionService;
    ...
    @GetMapping("/{billType}/{id}/events")
    @RequiresRole("admin")
    public ResponseEntity<List<BillEventResponseDTO>> getBillEvents(
            @PathVariable String billType,
            @PathVariable Long id,
            HttpServletRequest request) {
        String location = RequestUtil.getLocationFromRequest(request);
        return ResponseEntity.ok(billService.listBillEvents(id, billType, location));
    }
    ...
    public ResponseEntity<List<BillResponseDTO>> getAllBills(
        ...
        List<BillResponseDTO> bills = billService.getAllBills(location, createdBy, billDateFrom, billDateTo);
    ...
    public ResponseEntity<List<BillResponseDTO>> getAllSales(
            ...
            HttpServletRequest request) {
        String location = RequestUtil.getLocationFromRequest(request);
        List<BillResponseDTO> sales = billService.getAllSales(location, billDateFrom, billDateTo);
    ...
    public ResponseEntity<BillResponseDTO> addPaymentToExistingBill(
        ...
        String location = RequestUtil.getLocationFromRequest(request);
        Long userId = RequestUtil.getUserIdFromRequest(request);
        BillResponseDTO response = billRevisionService.adjustPayments(id, billType, paymentRequest, location, userId);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<BillResponseDTO> replaceExistingBill(
        ...
        Long userId = RequestUtil.getUserIdFromRequest(request);
        String role = RequestUtil.getRoleFromRequest(request);
        BillRevisionEditRequest edit = new BillRevisionEditRequest();
        edit.setMode(BillRevisionEditMode.FULL_REPLACE);
        edit.setBillId(id);
        edit.setBillType(billType);
        edit.setLocation(location);
        edit.setActorUserId(userId);
        edit.setActorRole(role);
        edit.setReplaceRequest(billRequestDTO);
        BillRevisionResultDTO result = billRevisionService.editBillWithIntegrity(edit);
        return ResponseEntity.ok(result.getBill());
    }
    ...
    @GetMapping("/{billType}/{id}/stock-returns")
    @RequiresRole({ "user", "admin" })
    public ResponseEntity<List<BillStockReturnHistoryDTO>> listStockReturns(
            @PathVariable String billType,
            @PathVariable Long id,
            HttpServletRequest requestHttp) {
        String location = RequestUtil.getLocationFromRequest(requestHttp);
        return ResponseEntity.ok(billService.listStockReturnsForBill(id, billType, location));
    }
    ...
    @Operation(summary = "Patch line quantities / add lines", description = "Difference-based edits. "
            + "Existing lines: quantity up (reserve + SALE/OUT + consume), down (RETURN/IN), or quantity 0 to remove line. "
            + "Case D — optional `addedItems`: append new bill lines; merged reserve → SALE/OUT (`source_action` BILL_LINE_NEW_ITEM_SALE) → consume. "
            + "After totals are recomputed from lines/charges, wallet advance is reversed and reapplied up to the new bill total (cash/UPI rows unchanged); use PUT replace to change payment split. "
            + "Send `lines` and/or `addedItems` (at least one required). GST stock/wallet applies only when the business flag enables GST inventory.")
    ...
    public ResponseEntity<BillResponseDTO> patchBillLineQuantities(
        ...
        Long userId = RequestUtil.getUserIdFromRequest(requestHttp);
        BillRevisionEditRequest edit = new BillRevisionEditRequest();
        edit.setMode(BillRevisionEditMode.LINE_QUANTITIES_PATCH);
        edit.setBillId(id);
        edit.setBillType(billType);
        edit.setLocation(location);
        edit.setActorUserId(userId);
        edit.setPatchRequest(request);
        BillResponseDTO dto = billRevisionService.editBill(edit);
        return ResponseEntity.ok(dto);
    }

    @Operation(summary = "Pre-flight stock for full bill replace", description = "Validates net quantity increases against on-hand stock before PUT replace. Does not mutate the bill.")
    @PostMapping("/{billType}/{id}/revision/stock-preflight")
    @RequiresRole({ "user", "admin" })
    public ResponseEntity<Void> revisionStockPreflight(
            @PathVariable String billType,
            @PathVariable Long id,
            @Valid @RequestBody BillRequestDTO billRequestDTO,
            HttpServletRequest request) {
        String location = RequestUtil.getLocationFromRequest(request);
        billRevisionService.adjustInventoryForFullReplaceProposal(id, billType, location, billRequestDTO);
        return ResponseEntity.noContent().build();
    }

    @Operation(summary = "Full revision preview", description = "Totals delta, settlement (additional payment / refund), stock warnings, and validation messages.")
    @PostMapping("/{billType}/{id}/revision/preview")
    @RequiresRole({ "user", "admin" })
    public ResponseEntity<BillRevisionPreviewDTO> revisionPreview(
            @PathVariable String billType,
            @PathVariable Long id,
            @Valid @RequestBody BillRequestDTO billRequestDTO,
            HttpServletRequest request) {
        String location = RequestUtil.getLocationFromRequest(request);
        return ResponseEntity.ok(billRevisionService.buildRevisionPreview(id, billType, location, billRequestDTO));
    }

    @Operation(summary = "Apply bill revision with integrity report", description = "Same as PUT replace but returns bill plus post-commit settlement integrity findings.")
    @PutMapping("/{billType}/{id}/revision")
    @RequiresRole({ "user", "admin" })
    public ResponseEntity<BillRevisionResultDTO> replaceBillWithRevisionResult(
            @PathVariable String billType,
            @PathVariable Long id,
            @Valid @RequestBody BillRequestDTO billRequestDTO,
            HttpServletRequest request) {
        ...
        edit.setActorRole(role);
        edit.setReplaceRequest(billRequestDTO);
        return ResponseEntity.ok(billRevisionService.editBillWithIntegrity(edit));
    }

    @Operation(summary = "Preview bill replace totals", description = "Compares current bill read model to totals implied by a replace body (same math as server replace).")
    @PostMapping("/{billType}/{id}/revision/difference")
    @RequiresRole({ "user", "admin" })
    public ResponseEntity<BillRevisionDifferenceDTO> revisionCalculateDifference(
            @PathVariable String billType,
            @PathVariable Long id,
            @Valid @RequestBody BillRequestDTO billRequestDTO,
            HttpServletRequest request) {
        String location = RequestUtil.getLocationFromRequest(request);
        BillResponseDTO current = billService.getBillById(id, billType, location);
        return ResponseEntity.ok(billRevisionService.calculateDifference(current, billRequestDTO, billType));
    }

    @Operation(summary = "Verify bill settlement integrity", description = "Read-only checks: amountDue vs total/advance/paid, payment row sum vs paidAmount.")
    @GetMapping("/{billType}/{id}/revision/integrity")
    @RequiresRole({ "user", "admin" })
    public ResponseEntity<BillRevisionIntegrityReportDTO> revisionVerifyIntegrity(
            @PathVariable String billType,
            @PathVariable Long id,
            HttpServletRequest request) {
        String location = RequestUtil.getLocationFromRequest(request);
        return ResponseEntity.ok(billRevisionService.verifyBillRevisionIntegrity(id, billType, location));
    }

    @Operation(summary = "Refresh bill payment status", description = "Recomputes paidAmount, paymentStatus, and payment summary from persisted bill_payments and advance (no deletes).")
    @PostMapping("/{billType}/{id}/revision/refresh-outstanding")
    @RequiresRole({ "admin" })
    public ResponseEntity<BillResponseDTO> revisionRefreshOutstanding(
            @PathVariable String billType,
            @PathVariable Long id,
            HttpServletRequest request) {
        String location = RequestUtil.getLocationFromRequest(request);
        Long userId = RequestUtil.getUserIdFromRequest(request);
        return ResponseEntity.ok(billRevisionService.adjustOutstanding(id, billType, location, userId));
    }

    @Operation(summary = "Resync wallet advance on bill", description = "Reverses and reapplies customer wallet advance against the current bill total (Non-GST; GST only when stock/wallet flag enabled).")
    @PostMapping("/{billType}/{id}/revision/resync-advance")
    @RequiresRole({ "admin" })
    public ResponseEntity<BillResponseDTO> revisionResyncAdvance(
            @PathVariable String billType,
            @PathVariable Long id,
            HttpServletRequest request) {
        String location = RequestUtil.getLocationFromRequest(request);
        Long userId = RequestUtil.getUserIdFromRequest(request);
        return ResponseEntity.ok(billRevisionService.adjustAdvance(id, billType, location, userId));
    }

    @Operation(summary = "Rebuild closing snapshots for bill", description = "Cascades daily closing snapshots from this bill's payment dates through today for the branch.")
    @PostMapping("/{billType}/{id}/revision/recompute-snapshots")
    @RequiresRole({ "admin" })
    public ResponseEntity<Void> revisionRecomputeSnapshots(
            @PathVariable String billType,
            @PathVariable Long id,
            HttpServletRequest request) {
        String location = RequestUtil.getLocationFromRequest(request);
        billRevisionService.recomputeSnapshots(id, billType, location);
        return ResponseEntity.noContent().build();
    }

    @Operation(summary = "Preview bill cancellation", description = "Shows refund, stock restore, and advance impact before confirming cancel.")
    @GetMapping("/{billType}/{id}/cancel-preview")
    @RequiresRole({ "user", "admin" })
    public ResponseEntity<BillCancelPreviewDTO> getBillCancelPreview(
            @PathVariable String billType,
            @PathVariable Long id,
            HttpServletRequest request) {
        String location = RequestUtil.getLocationFromRequest(request);
        return ResponseEntity.ok(billCancellationService.getCancelPreview(id, billType, location));
    }

    @Operation(summary = "Cancel finalized bill", description = "Reverses stock, payments, and advance; preserves audit history.")
    @PostMapping("/{billType}/{id}/cancel")
    @RequiresRole({ "user", "admin" })
    public ResponseEntity<Void> cancelBill(
        ...
        String location = RequestUtil.getLocationFromRequest(request);
        Long userId = RequestUtil.getUserIdFromRequest(request);
        billCancellationService.cancelBill(id, billType, location, userId, cancelBody);
        return ResponseEntity.noContent().build();
    }
    ...
    public ResponseEntity<Void> deleteBillById(
        ...
        billCancellationService.cancelBillById(id, location, userId, cancelBody);
        return ResponseEntity.noContent().build();
    }
