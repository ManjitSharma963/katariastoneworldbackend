...
/**
 * Legacy {@code transactions} upsert/delete without accounting-bridge dependencies.
 * Used by accounting bridges for flag-off fallback so they do not depend on {@link MoneyTransactionService}.
 */
@Service
@Transactional
public class MoneyTransactionLegacySync {

    private static final int NOTES_MAX = 2000;

    private final MoneyTransactionRepository moneyTransactionRepository;

    public MoneyTransactionLegacySync(MoneyTransactionRepository moneyTransactionRepository) {
    ...
    public void hardDeleteSyncedLine(String source, Long referenceId) {
        if (source == null || source.isBlank() || referenceId == null) {
            return;
        }
        MoneyCategory category = categoryFromSource(source);
        MoneyReferenceType referenceType = referenceTypeFromSource(source);
    ...
    public void recordReceivableRepaymentReceived(
            ReceivableLedgerEntry entry, LoanBorrower borrower, String normalizedPaymentMode) {
        if (entry == null || entry.getId() == null || borrower == null || entry.getAmount() == null
                || entry.getAmount().compareTo(BigDecimal.ZERO) <= 0) {
            return;
        }
    ...
    public static MoneyCategory categoryFromSource(String source) {
        ...
        return switch (s) {
            ...
            case "BILL_RETURN_WALLET_CREDIT" -> MoneyCategory.OTHER;
    ...
    public static MoneyReferenceType referenceTypeFromSource(String source) {
        String s = source.trim().toUpperCase(Locale.ROOT);
        return switch (s) {
            case "BILL", "ADVANCE" -> MoneyReferenceType.bill;
            case "ADVANCE_REFUND", "BILL_EDIT_ADJUSTMENT", "BILL_RETURN_WALLET_CREDIT" -> MoneyReferenceType.other;
