...
public class PdfService {
    ...
    private String populateTemplate(String template, BillResponseDTO bill, Seller seller) {
        ...
        String sellerMobile = seller.getMobile() != null && !seller.getMobile().trim().isEmpty() ? seller.getMobile()
                : "-";
        String sellerName = seller.getSellerName() != null && !seller.getSellerName().trim().isEmpty()
                ? seller.getSellerName()
                : "-";
        String sellerAddress = seller.getAddress() != null && !seller.getAddress().trim().isEmpty()
                ? seller.getAddress().replace("\n", "<br />")
                : "-";
        ...
        String termsAndConditions = seller.getTermsAndConditions() != null
        ...
        if (stateOfOrigin != null && !stateOfOrigin.isEmpty()) {
            StateGstMaster sellerStateGst = stateGstMasterRepository.findByStateNameIgnoreCase(stateOfOrigin)
                    .orElse(null);
        ...
        if (stateOfDestination != null && !stateOfDestination.isEmpty()) {
            StateGstMaster buyerStateGst = stateGstMasterRepository.findByStateNameIgnoreCase(stateOfDestination)
                    .orElse(null);
        ...
        html = html.replace("{{stateOfOrigin}}",
        ...
        if (stateOfOrigin != null && !stateOfOrigin.isEmpty() && stateOfDestination != null
                && !stateOfDestination.isEmpty()) {
        ...
        html = html.replace("{{customerName}}",
                bill.getCustomerName() != null ? escapeHtml(bill.getCustomerName()) : "");
        html = html.replace("{{address}}", bill.getAddress() != null ? escapeHtml(bill.getAddress()) : "");
        html = html.replace("{{customerMobileNumber}}",
                bill.getCustomerMobileNumber() != null ? bill.getCustomerMobileNumber() : "");
        ...
        // Amounts
        html = html.replace("{{subtotal}}",
                bill.getSubtotal() != null ? DECIMAL_FORMAT_NO_DECIMAL.format(bill.getSubtotal()) : "0");
        html = html.replace("{{taxPercentage}}",
                bill.getTaxPercentage() != null ? DECIMAL_FORMAT_NO_DECIMAL.format(bill.getTaxPercentage()) : "0");
        html = html.replace("{{taxAmount}}",
                bill.getTaxAmount() != null ? DECIMAL_FORMAT_NO_DECIMAL.format(bill.getTaxAmount()) : "0");
        ...
        html = html.replace("{{transportationCharge}}",
                bill.getTransportationCharge() != null ? DECIMAL_FORMAT.format(bill.getTransportationCharge())
                        : "0.00");
        html = html.replace("{{otherExpenses}}",
                bill.getOtherExpenses() != null ? DECIMAL_FORMAT.format(bill.getOtherExpenses()) : "0.00");
        html = html.replace("{{totalAmount}}",
                bill.getTotalAmount() != null ? DECIMAL_FORMAT_NO_DECIMAL.format(bill.getTotalAmount()) : "0");
        ...
        // Handle conditional sections - remove conditionals and keep content if
        // condition is true
        ...
        // Other expenses
        if (bill.getOtherExpenses() == null || bill.getOtherExpenses() == 0) {
            html = removeConditionalBlock(html, "{{#if otherExpenses}}", "{{/if}}");
        } else {
            html = removeConditionalBlock(html, "{{#if otherExpenses}}", "{{/if}}", true);
        }
        ...
        if (bill.getItems() != null && !bill.getItems().isEmpty()) {
            ...
            for (BillItemDTO item : bill.getItems()) {
                ...
                itemsHtml.append("<td class=\"text-right\">").append(DECIMAL_FORMAT.format(quantity)).append(" (")
                        .append(item.getUnit() != null ? item.getUnit() : "").append(")</td>");
                itemsHtml.append("<td class=\"text-right\">").append(DECIMAL_FORMAT.format(pricePerUnit))
                        .append("</td>");
        ...
        // Final cleanup - remove any remaining template markers (but not already
        // replaced values)
    ...
    private String populateSimpleTemplate(String template, BillResponseDTO bill) {
        ...
        html = html.replace("{{customerMobileNumber}}",
                bill.getCustomerMobileNumber() != null ? bill.getCustomerMobileNumber() : "");
        html = html.replace("{{discountAmount}}",
                bill.getDiscountAmount() != null ? DECIMAL_FORMAT.format(bill.getDiscountAmount()) : "0.00");
        // Replace items section
        StringBuilder itemsHtml = new StringBuilder();
        if (bill.getItems() != null && !bill.getItems().isEmpty()) {
            int srNo = 1;
        ...
        // Total amount
        html = html.replace("{{totalAmount}}",
                bill.getTotalAmount() != null ? DECIMAL_FORMAT.format(bill.getTotalAmount()) : "0.00");
