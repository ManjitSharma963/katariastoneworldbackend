...
@Entity
@Table(name = "bill_items_gst")
@Data
@NoArgsConstructor
@AllArgsConstructor
public class BillItemGST {
    ...
    @JoinColumn(name = "bill_id", nullable = false, foreignKey = @ForeignKey(name = "fk_bill_item_gst_bill", value = ConstraintMode.NO_CONSTRAINT))
    private BillGST bill;
    
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "product_id", nullable = true, foreignKey = @ForeignKey(name = "fk_bill_item_gst_product", value = ConstraintMode.NO_CONSTRAINT))
